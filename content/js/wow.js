			...
			if( http ) {
				...
				http.onreadystatechange = function () {
					if(http.readyState == 4) {
							text_to_append = http.responseText;
							$(".tooltip-content").html(http.responseText);
					}
				}
				http.send(null);    
			}
			$('<li/>').append($('<h3/>').html(text_to_append)).appendTo($tooltip);
			
		}

		if (description) {
			$('<li/>').addClass('color-tooltip-yellow').html('<div style="width: 280px">' + description + '</div>').appendTo($tooltip);
		}
...
Wow.Icon = {
	...
	getUrl: function(name, size) {
		...
		return Core.cdnUrl +'/wow/icons/'+ iconSize +'/'+ name +'.jpg';
	},
	...
	framedIcon: function(name, size) {
		...
		if (size == 18 || size == 36 || size == 56) {
			$icon.css('background-image', 'url(' + Wow.Icon.getUrl(name, size) + ')');
		} else {
